...
TP = zeros(F_fold, size(params.k_range, 2));
FP = zeros(F_fold, size(params.k_range, 2));
avgTP = zeros(1, size(params.k_range, 2));
avgFP = avgTP;
stdFP = avgTP; 
stdTP = avgTP;

%splitting data each time with a different randomization and performing
%knn_ROC each time in a different row
for k=1:F_fold
    [X_train, y_train, X_test, y_test] = split_data(X, y, valid_ratio);
    [TP(k, :), FP(k, :)] = knn_ROC(X_train, y_train, X_test, y_test, params);
end

%taking the mean and std of each column
